 ...
 * Mogees Test
 * usage:
 * javac Triangle_Divisors_Impv.java
 * java Triangle_Divisors_Impv 500
 * @author Angelo Kaichen Huang
 */
public class Triangle_Divisors_Impv {
	...
	*/
	...
	private static Map<Integer, Integer> factorize(int num)
	{
        ...
        int twoFacts = 0; //calc 2's factors first
        while(num%2==0){
        	num/=2;
        	twoFacts++;
        }
        if (twoFacts > 0)
        	primeFactors.put(2, twoFacts);
        
        int div = 3; //initialize to 3 *****IMPROVED*****
        while (num > 1)  //While num is more than one
        {
			...
			if (isPrime(div) && (num % div == 0)) //If 'div' is a prime number AND 'num' modulus 'div' equals zero
            {
                ...
                div+=2; //increase 'div' by 2   *****IMPROVED*****
